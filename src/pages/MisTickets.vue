<template>
  <div class="h-full overflow-auto relative">
    ...
    <div class="mx-auto max-w-6xl w-full px-4 sm:px-8 py-10 mt-10">
      <!-- CARD principal -->
      <!-- CARD -->
    <div class="relative bg-white shadow-md rounded-2xl p-10 overflow-hidden w-full min-w-[420px] max-w-[620px] mx-auto flex flex-col gap-6 border border-[#01C38E] transition-all duration-300">
    <!-- Título -->
    ...
    <div>
        ...
        <select
        id="tipo"
        v-model="nuevoTicket.tipo_soporte"
    ...
    >
      ...
      <!-- Tickets del usuario -->
      <div v-if="tickets.length" class="mt-10 max-w-xl mx-auto">
          ...
          >
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-gray-500">ID: {{ ticket.id.slice(0, 8) }} · {{ ticket.estado }} · {{ ticket.tipo_soporte }}</p>
              </div>
              <span class="text-xs text-gray-400">{{ formatDate(ticket.actualizado) }}</span>
            </div>
          </li>
        </ul>
      </div>
...
const user = ref({});
const nuevoTicket = ref({ tipo_soporte: 'Remoto', descripcion: '' });
...
const crearTicket = async () => {
  ...
  const { error } = await supabase.from('tickets').insert({
    ...
    tipo_soporte: nuevoTicket.value.tipo_soporte,
