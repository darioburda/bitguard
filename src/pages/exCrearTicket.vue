<template>
  <div class="max-w-screen-md mx-auto px-4 py-10">
    ...
    <form @submit.prevent="submitForm" class="space-y-6">
      ...
      <!-- Usuario solicitante (solo admin) -->
...
export default {
  ...
  setup() {
    ...
    const cargarDatos = async () => {
      ...
      const perfil = await getUserProfileById(authUser.user.id);
      user.value = perfil;
      ...
      if (isAdmin.value) {
        ...
        ticket.value.usuario_id = perfil.id;
        ticket.value.empresa_id = perfil.empresa_id;
        const perfiles = await getAllUserProfiles();
        tecnicos.value = perfiles.filter(u => u.is_admin);
      }
    };
