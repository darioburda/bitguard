...
export async function getUserProfilesPaginated(limit = 10, page = 1) {
  ...
  return {
    data: data.map(profile => ({
      ...
      plan_nombre: profile.empresas?.plan?.nombre || null,
      photo: profile.photo || defaultPhotoUrl
    })),
    count
  }
...
export async function getUserProfileByDisplayName(displayName) {
  const { data, error } = await supabase
    .from('user_profiles')
  ...
  if (error) {
    console.error('[user-profiles.js getUserProfileByDisplayName] Error al buscar nombre:', error)
    throw error
  }
